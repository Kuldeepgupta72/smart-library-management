...
## Hooks
Every agent/subagent applies on_start (verify Input files exist)
and on_complete (append to docs/{{STORY_ID}}/pipeline-log.md).
on_complete is batched: one write per top-level agent, not per
internal stage/subagent. Full definition:
.github/hooks/pipeline-hooks.md

## Rules
Every agent/subagent follows the shared, numbered guardrail set in
.github/rules/pipeline-rules.md (Jira read-only, GitHub/Confluence
write boundaries, data integrity, security, checkpoints, scope
discipline) instead of repeating rules inline per agent.
...
- App repo: feature/copilot-{{STORY_ID}}-{{short-description}}
  Example: feature/copilot-AISDLC-1-payment-gateway
...
.github/hooks/pipeline-hooks.md), giving a running audit trail of
which stage ran when, what it produced, and its checkpoint result.
