---
description: Run the Agentic SDLC pipeline for a Jira story end to end, starting from Stage 1a. Leave the argument empty to browse the AISDLC backlog instead of naming a story.
argument-hint: [AISDLC-{{NUMBER}}]
---

Run the full Agentic SDLC pipeline for Jira story `$ARGUMENTS`
yourself, in this session — do not spawn a separate "orchestrator"
subagent. You (the main session) directly Task-invoke each top-level
stage agent below, in order, the same way `docs-agent.md` already
Task-invokes its own 3 subagents one level down. A called subagent
cannot see this session's conversation history, so pass it context
explicitly in the Task invocation.

- If an argument was given, it is a story ID in the form
  AISDLC-{{NUMBER}}: `$ARGUMENTS`. Task-invoke `jira-agent`
  (`.claude/agents/jira-agent.md`) directly in Single Story Mode with
  that ID.
- If no argument was given, Task-invoke `jira-agent` in Backlog Mode
  so it lists the open AISDLC stories for the human to choose from.
- Optionally, if there's no existing story yet, the human may
  describe a gap/enhancement directly in chat to trigger `jira-agent`'s
  optional Stage 0 (human-approved Jira story creation) first — it
  hands off into the same Stage 1a flow once the story exists. The
  human can also explicitly ask to scan the repo for enhancements to
  trigger `gap-scanner-agent` — or it triggers automatically if
  Backlog Mode's live query comes back with zero open stories.
...
## All Stage Agents (Task-invoke directly, in this order)

-   Gap Scanner (optional before Stage 0, auto-triggered by Stage 1a
    Backlog Mode on zero open stories): `.claude/agents/gap-scanner-agent.md`
...
Each stage agent owns and executes its own checkpoint directly (asks
the human in chat, waits for APPROVE/REJECT) — this session doesn't
intercept or re-ask on their behalf, just waits for each stage to
report its result before invoking the next one.
...
3.  That command invokes the "Next Action" agent it names via the Task tool directly — no orchestrator involved. See `.claude/hooks/log-pipeline-stage.js` for exactly how that file is written.

## Rules

See `AGENTS.md`'s Pipeline Rules section — applies to every stage. This
session never calls Jira/GitHub/Confluence directly itself while
running the pipeline; it only Task-invokes the stage agents that do.

## Hooks

Real hooks configured in `.claude/settings.json` (`.claude/hooks/check-prereqs.js`, `.claude/hooks/log-pipeline-stage.js`). This session does not apply on_start/on_complete itself — each delegated stage agent/subagent applies its own (automatically for the three documentation subagents via the hook scripts; explicitly via `file-writer` for the rest), so docs/{{STORY_ID}}/pipeline-log.md ends up with one row per stage regardless of which agent ran it.
