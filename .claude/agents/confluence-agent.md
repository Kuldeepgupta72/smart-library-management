...
3. Compile page content:
   ...
   - Design Doc Link — the Confluence Design page URL published by
     ...
     `docs/{{STORY_ID}}/design-{{STORY_ID}}.md` file/PR reference
     only if that page link is genuinely unavailable (mark
     `[pending]` rather than guessing a URL)
...
(never invent facts — mark genuinely missing info [pending]).
