...
and never touches Jira/GitHub/Confluence credentials or APIs.
...
1. Check grounded sources for gaps, in this order:
   ...
   - `TODO`/`FIXME`/`XXX` comments anywhere in `src/`, `public/`,
     `tests/`
...
## Next Stage
`jira-agent` (`.claude/agents/jira-agent.md`) — Stage 0, once per
picked candidate
